...
if st:
    ...
    print(f"Waveform Start Time = {t2}")
    print(f"Waveform End Time = {t3}", "\n")
...
print("model params input :", params["model_stats"])
model.load_state_dict(torch.load(params["model_stats"]))
...
categories = {
    "Label: 0, Predict: 0": {"examples": classified_as_00, "color": "#1f77b4", "name": "00"},
    "Label: 1, Predict: 1": {"examples": classified_as_11, "color": "#1f77b4", "name": "11"},
    "Label: 0, Predict: 1 (Misclassified)": {"examples": misclassified_as_01, "color": "#d62728", "name": "01"},
    "Label: 1, Predict: 0 (Misclassified)": {"examples": misclassified_as_10, "color": '#d62728', "name": "10"}
}

# for title, category_info in categories.items():
#     examples = category_info["examples"]
#     color = category_info["color"]
#     folder_name = category_info["name"]
#     sub_folder_path = os.path.join("tmp", folder_name)
#     if not os.path.exists(sub_folder_path):
#         os.makedirs(sub_folder_path)
# 
#     for data, label, predict, station_name, channel_name in examples:
...
# 
#         plt.tight_layout()
#         file_name = f"{station_name}_{channel_name}.png"
#         file_path = os.path.join(sub_folder_path, file_name)
#         plt.savefig(file_path, dpi=300)
# 
#         plt.close(fig)

# print("All waveforms have been plotted and saved.")
...
plt.savefig("I_CNN_test_examples_overview_TEST.png", dpi=300)
...
if not all_labels or not all_predictions:
    ...
    plt.tight_layout()
    plt.savefig("I_CNN_confusion_matrix_TEST.png", dpi=300, bbox_inches='tight', pad_inches=0.1)
